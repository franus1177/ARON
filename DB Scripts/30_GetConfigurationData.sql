...
BEGIN
	...
	IF @LangCode	= 'EN' 
	BEGIN
		INSERT INTO	@Language	VALUES	(	@LangCode, 'English'	);
		--INSERT INTO	@Language	VALUES	(	@LangCode, 'Swedish'	);
	END
...
BEGIN
		...
		BEGIN
			...
			--INSERT INTO	@Module	VALUES	(	@LangCode,	'DR',	'Drill'		 ,	1,	4	);
			--INSERT INTO	@Module	VALUES	(	@LangCode,	'CM',	'Compliance' ,	1,	5	);
			--INSERT INTO	@Module	VALUES	(	@LangCode,	'RK',	'Risk'	     ,	1,	6	);
			--INSERT INTO	@Module	VALUES	(	@LangCode,	'CM',	'Compliance'	,	1,	5	);
			--INSERT INTO	@Module	VALUES	(	@LangCode,	'RK',	'Risk Management'	,	1,	6	);
		END
...
BEGIN
	...
	IF @LangCode	= 'EN' 
	BEGIN
		INSERT INTO	@Country	VALUES	(	@LangCode,	'BHR',	'Bahrain'				);
		INSERT INTO	@Country	VALUES	(	@LangCode,	'UAE',	'United Arab Emirates'	);
		INSERT INTO	@Country	VALUES	(	@LangCode,	'SA',	'Saudi Arabia'			);
		INSERT INTO	@Country	VALUES	(	@LangCode,	'QTR',	'Qatar'					);
		INSERT INTO	@Country	VALUES	(	@LangCode,	'KWT',	'Kuwait'				);
	END
...
BEGIN
	...
	BEGIN
		...
		INSERT INTO @RiskLevel	VALUES	(	2,	@LangCode,	'Medium','#FA8072'	);
		INSERT INTO @RiskLevel	VALUES	(	3,	@LangCode,	'Low',   '#99FF33'	);
...
BEGIN
	...
	IF @LangCode	= 'EN' 
	BEGIN
		INSERT INTO @InspectionType	VALUES	(	@LangCode,	'AMC'		, 'AMC'			);
		INSERT INTO @InspectionType	VALUES	(	@LangCode,	'Emergency'	, 'Emergency'	);
		INSERT INTO @InspectionType	VALUES	(	@LangCode,	'Resolution', 'Resolution'	);
	END
...
BEGIN
	...
	BEGIN
		...
		INSERT INTO @InspectionType	VALUES	(	@LangCode, 'PreFirePlan', 'Pre Fire Plan'	);
		--INSERT INTO @InspectionType	VALUES	(	@LangCode, 'AMC', 'AMC'	);
		--INSERT INTO @InspectionType	VALUES	(	@LangCode, 'Emergency', 'Emergency'	);
		--INSERT INTO @InspectionType	VALUES	(	@LangCode, 'Routine', 'Routine'	);
		--INSERT INTO @InspectionType	VALUES	(	@LangCode, 'Resolution', 'Resolution'	);
		--INSERT INTO @InspectionType VALUES ( @LangCode, 'Aircraft', 'Aircraft Incident' );
		--INSERT INTO @InspectionType VALUES ( @LangCode, 'FRAS', 'Fire Risk Assessment' );
		--INSERT INTO @InspectionType VALUES ( @LangCode, 'Drill', 'Drill' );
	END
	ELSE IF @LangCode	= 'SV' 
	BEGIN
		INSERT INTO @InspectionType	VALUES	(	@LangCode, 'Routine', 'Rutininspektion'	);
		INSERT INTO @InspectionType	VALUES	(	@LangCode, 'PreFirePlan', 'Pre Fire Plan'	);
		--INSERT INTO @InspectionType	VALUES	(	@LangCode, 'Emergency', 'Emergency'	);
	END
...
BEGIN
	...
	IF @LangCode	= 'EN' 
	BEGIN
		INSERT INTO @ServiceLevel	VALUES	(	1,	@LangCode,	'NFPA'/*'Standard'*/	);-- NFPA- For all (National Fire protection act.)
		INSERT INTO @ServiceLevel	VALUES	(	2,	@LangCode,	'DCD'/*'Non-standard'*/	);--For only Dubai
	END
...
BEGIN
	...
	DECLARE	@Configuration	TABLE	(	ConfigurationCode		ConfigurationCode	NOT NULL	PRIMARY KEY,
										ConfigurationValue		INTEGER					NULL,
										CanBeSetForCustomer		BIT					NOT NULL
									);

	INSERT	INTO	@Configuration	VALUES	(	'CtgryLvl',			4,		0);			--- Maximum Number of Levels allowed for Categories across Business Lines
	...
	--INSERT	INTO	@Configuration	VALUES	(	'NxtDaysData',		15,		1);		--- Number of days in future for which data should be loaded on a device
	...
	DECLARE	@ConfigurationName	TABLE	(	ConfigurationCode	ConfigurationCode	NOT NULL,
											LanguageCode		LanguageCode		NOT	NULL,
	...
	-- ======================================================================
		...
		ORDER BY a.SeverityLevelID;

	-- ======================================================================
	---------------------- Inspection Types --------------------------------- 
	-- ======================================================================

		SELECT	a.InspectionType,a.InspectionTypeName
		FROM	dbo.__GetConfigurationInspectionType(@L) as a
		ORDER BY a.InspectionType;
	...
	-- ======================================================================
		...
		FROM	dbo.__GetConfigurationServiceLevel(@L) as a
		ORDER BY a.ServiceLevelID;
...
BEGIN
	...
	-- ======================================================================
		...
		SELECT	*
		FROM	dbo.__GetConfigurationModule(@L)
		WHERE	LanguageCode	=	@L
		ORDER BY	SequenceNo;
