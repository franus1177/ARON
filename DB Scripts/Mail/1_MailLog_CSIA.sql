...
(
	@p_From					NVARCHAR(50),
	@p_RecipientMailList	VARCHAR(4000),
	@p_CCMailList			VARCHAR(4000),
	@p_BCCMailList			VARCHAR(4000),
...
BEGIN
	...
	if(@p_From is not null)
		SET @p_From = @p_From + ' rajendra.more@convenio.com';

		EXEC msdb.dbo.sp_send_dbmail
				@profile_name			=	'CSIA_Mail', 
		...
		--===Below Commnet Code is Generate New Profile--and if want to update profile then drop first and recreate==========================================
			...
			--declare @profileName2 varchar(100) = 'CSIA_Mail'
			--declare @account_name2 varchar(100) = @profileName2 +'_Account'

			--declare @account_name            varchar(100) = @account_name2,
			--		@email_address           varchar(100) = 'rajendra.more@convenio.com', 
			--		@display_name            varchar(100) = 'CSIA Alert', 
			--		@replyto_address         varchar(100) = '', 
			--		@description             varchar(100) = '', 
			--		@mailserver_name         varchar(100) = 'smtp.office365.com', 
			--		@mailserver_type         varchar(100) = 'SMTP', 
			--		@port                    varchar(100) = '587', 
			--		@username                varchar(100) = 'rajendra.more@convenio.com', 
			--		@password                varchar(100) = 'raju.more999@mca',  
			--		@use_default_credentials bit =  0 , 
			--		@enable_ssl              bit =  1 ; 


			----==Drop Settings For eSafecCSIA_Mail Profile ==================================================================
			...
			--			WHERE p.name = 'CSIA_Mail'  AND a.name = 'CSIA_Mail_Account') 
			--BEGIN 
			...
			----==end Drop Settings For eSafecCSIA_Mail Profile ==================================================================
				

			----===========================================================================================================
			----== BEGIN Mail Settings eSafecCSIA_Mail 
			----=============================================================================================================
			--IF NOT EXISTS(SELECT * FROM msdb.dbo.sysmail_profile WHERE  name = @profileName2)  
			--BEGIN 
  
			----CREATE Profile [eSafecCSIA_Mail] 
			...
			--BEGIN 
			--	--CREATE Account [eSafecCSIA_MailAccount] 
			...
			--	BEGIN 
			--			-- Associate Account [eSafecCSIA_MailAccount] to Profile [eSafecCSIA_Mail] 
		...
		--	exec msdb.dbo.sp_send_dbmail @profile_name = 'CSIA_Mail', 
		--  @recipients = 'rajendra.more999@gmail.com', @subject = 'Mail Test', 
		--  @body = 'Mail Sent Successfully', @body_format = 'HTML'
		--- 1 send , 2 fail, 3 retry
		--	select sent_status,* from msdb.[dbo].[sysmail_mailitems] order by send_request_date desc
