...
var customer = {
    ...
    SetForAdd: function () {
        ...
        LoadAddUpdateView(divAddUpdateID, "/Customer/_partialAddUpdate", Resource.Add,
            function () {
                customer.ClearData();
                customer.AddEvents();
    ...
    AddUpdate: function () {
        Reset_Form_Errors();
        var FormJsonData = customer.GetData();
        if (customer.ValidateData(FormJsonData)) {
            ...
            if (window.FormData !== undefined) {
                ...
                fileData.append('Model', JSON.stringify(FormJsonData));
            }

            $.ajax({
                type: "POST", /*cache: false,*/ url: "/Customer/AddUpdateData", data: fileData,
                contentType: false, /* Not to set any content header*/processData: false, /* Not to process data*/
                    ...
                    if (data != null) {
                        ...
                        if (data.Status == 'Success') {
                            if (eval(data.Data) > 0) {
                                ...
                                if (FormJsonData.CustomerID == null || FormJsonData.CustomerID == 0 || FormJsonData.CustomerID == "" || FormJsonData.CustomerID == undefined)
                                    $("#hf_CustomerID").val(data.Data);
                            ...
                            if (data.Message.toString().indexOf("Customer_UK_CustomerName") >= 0) {
                                Validate_Alert("#txtCustomerName", null, false);
                                $("#txtCustomerName").focus();
                                toastr.error(CustomerNameIsAlreadyExists);
                            } else if (data.Message.toString().indexOf("Customer_UK_CustomerCode") >= 0) {
                                Validate_Alert("#txtCustomerCode", null, false);
                                $("#txtCustomerCode").focus();
                                toastr.error(CustomerCodeIsAlreadyExists);
                            }
                            else if (data.Message.toString().indexOf("Customer_CK_EffectiveFromDate_EffectiveTillDate") >= 0) {
                                Validate_Alert("#txtEffectiveFromDate", null, false);
                                $("#txtEffectiveFromDate").focus();
                                toastr.error(FromDateisGreaterMessage);
                            }
                            else
                                toastr.error(data.Message);
                        }
